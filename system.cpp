...
public:
	...
	virtual Matrix  get_input() const= 0;
	virtual Matrix  get_output() const= 0;
...
public:
	...
	Matrix function(Matrix input) {
		...
		for (int j = 0; j < output.getRows(); j++) {
			for (int i = 0; i < output.getCols(); i++) {
				output[j][i] = linear_output[j][i] > 0 ? linear_output[j][i] : 0;
			}
		}
		layer_output = output;
		return output;
...
public:
	...
	Matrix backward( Matrix target, Matrix output,double lr) {
		Matrix error = output - target;
		Matrix current_output = output;
		for (int i = layer_num - 1; i >= 0; i--) {
			auto* layer = layers[i];
			Matrix& w = layer->get_w();
			Matrix& b = layer->get_b();
			Matrix current_input = layer->get_input();
			...
			for (int j = 0; j < error.getCols(); j++) {
				...
				dj_db[0][j] /= error.getCols();
			}
			Matrix grad_w = current_input.transpose() * error ;
				...
				if (typeid(*layer) == typeid(Sigmod)) {
					...
					for (int r = 0; r < error.getRows(); r++) {
						for (int c = 0; c < error.getCols(); c++) {
							double value = 1 / (1 + exp(-current_output[r][c]));
					...
					for (int r = 0; r < error.getRows(); r++) {
						for (int c = 0; c < error.getCols(); c++) {
							error[r][c] *= (current_output[r][c] > 0 ? 1 : 0);
						}
					}
				}
				Matrix d = w.transpose();
				error = error * d; // attention
			}
...
int main(){
	...
	// 创建神经网络
	Sequential model;
	model.add_layer(new Linear(3, 9));
	model.add_layer(new Sigmod(9, 4));
	...
	// 反向传播
	model.backward(target, output, 0.01);
